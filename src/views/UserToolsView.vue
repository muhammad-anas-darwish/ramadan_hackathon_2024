  ...
  <article v-else>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2 lg:gap-4 p-3 w-full">
      <div v-for="card in cardsData" :key="card.id" class="w-full border rounded-lg shadow bg-gray-800 border-gray-700">
        ...
        <div class="px-5 pb-5">
          <RouterLink class="block py-2 px-3 rounded md:p-0 text-white md:hover:text-blue-500 hover:bg-gray-700 hover:text-white md:hover:bg-transparent" :to="{ name: 'Tool',  params: { id: card.toolId } }"> 
            <h5 class="text-xl font-semibold tracking-tight text-gray-200">
              {{ card.title }}
            </h5>
          </RouterLink>
          <div class="mt-2.5 mb-2.5 break-words text-xl text-gray-400">{{ card.baseQuantity }}/{{ card.quantity }} قطع</div>
